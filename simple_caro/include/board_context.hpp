...
class Board_Context {
private:
    std::shared_ptr<std::vector<std::vector<TILE_STATE>>> mBoard;
    long mOccupied_tiles_counter;
...
public:
    ...
    MOVE_RESULT set_tile(Coordinate pos, TILE_STATE state);
    MOVE_RESULT unset_tile(Coordinate pos);
